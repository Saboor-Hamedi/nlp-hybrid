...
window.setMode = function(mode) {
    ...
    document.getElementById('stream-query').placeholder = mode === 'local' ? "RAG Assistance..." : "Global Assistance...";
};
...
async function executeStreamSearch() {
    ...
    try {
        ...
        if (error.name === "AbortError") {
            contentEl.innerHTML += '<p class="text-[10px] italic opacity-40 mt-4">[Synthesis Terminated]</p>';
        } else {
...
window.openForensicEditor = function(docId) {
    if (window.openEditor) {
        fetch(`/api/docs/${docId}`).then(r => r.json()).then(data => {
...
window.purgeForensicRecord = async function(docId) {
    if (!confirm('Are you sure you want to permanently PURGE this forensic record?')) return;
    try {
...
async function inspectDocument(id) {
    ...
    inspector.innerHTML = `
        <div class="flex flex-col items-center justify-center h-64">
            ...
            <p class="text-[9px] font-bold uppercase tracking-widest mt-4 opacity-40">Inspecting #${id}...</p>
        </div>
    `;
            ...
            <div class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="p-3 rounded-lg border theme-border bg-purple-500/5">
                        ...
                        <p class="text-[9px] font-bold theme-text">${data.lda_topic_label || 'Unclassified'}</p>
                    </div>
                    <div class="p-3 rounded-lg border theme-border bg-indigo-500/5">
                        ...
                        <p class="text-[9px] font-bold theme-text">${data.bert_topic_label || 'Unclassified'}</p>
                    </div>
                </div>
