...
function setMode(mode) {
    ...
    if (mode === 'local') {
        localBtn.className = 'px-2 py-1 rounded-md text-[7px] font-bold uppercase tracking-widest transition-all bg-blue-600 text-white shadow-sm';
        globalBtn.className = 'px-2 py-1 rounded-md text-[7px] font-bold uppercase tracking-widest transition-all theme-text opacity-40 hover:opacity-100';
        queryInput.placeholder = 'Search archive...';
        btn.className = 'w-8 h-8 bg-blue-600 text-white rounded-full flex items-center justify-center hover:bg-blue-700 active:scale-90 transition-all shadow-lg';
    } else {
        globalBtn.className = 'px-2 py-1 rounded-md text-[7px] font-bold uppercase tracking-widest transition-all bg-purple-600 text-white shadow-sm';
        localBtn.className = 'px-2 py-1 rounded-md text-[7px] font-bold uppercase tracking-widest transition-all theme-text opacity-40 hover:opacity-100';
        queryInput.placeholder = 'Ask Global...';
        btn.className = 'w-8 h-8 bg-purple-600 text-white rounded-full flex items-center justify-center hover:bg-purple-700 active:scale-90 transition-all shadow-lg';
    }
}
...
async function executeStreamSearch() {
    ...
    userMsg.innerHTML = `
        ...
        <span class="text-[8px] font-bold theme-text-sec opacity-30 uppercase tracking-widest mt-1 mr-1">${currentMode === 'local' ? 'Investigator' : 'Global User'}</span>
    `;
    thread.appendChild(userMsg);
    queryInput.value = '';
    queryInput.style.height = '36px'; // Reset to new ultra-compact height
    ...
    const label = currentMode === 'local' ? 'Neural Archive Analysis' : 'DeepSeek Global Intelligence';
    ...
    try {
        ...
        if (currentMode === 'local') {
            const searchResponse = await fetch(`/api/search?query=${encodeURIComponent(query)}`, { signal: abortController.signal });
            searchData = await searchResponse.json();
            if (searchData.results.length > 0) {
                const grid = document.getElementById(`${aiMsgId}-grid`);
                document.getElementById(`${aiMsgId}-evidence`).classList.remove('hidden');
                grid.innerHTML = searchData.results.slice(0, 4).map(res => {
        ...
        const synthResponse = await fetch('/api/synthesis', {
            ...
            body: JSON.stringify({ query: query, context_docs: searchData.results, mode: currentMode }),
        ...
        if (error.name === 'AbortError') contentEl.innerHTML += '<p class="text-[10px] italic opacity-40 mt-4">[Synthesis Terminated]</p>';
...
async function saveInsight(aiMsgId, query, mode) {
    ...
    try {
        ...
        if (data.status === 'success') {
            btn.className = 'px-3 py-1.5 bg-green-500 text-white rounded-md text-[7px] font-bold uppercase tracking-widest shadow-sm flex items-center gap-2';
...
async function inspectDocument(id) {
    ...
    try {
                    ...
                    <div class="space-y-4">
                        <div class="p-4 rounded-xl border theme-border bg-blue-600/5">
                            ...
                            <div class="flex flex-wrap gap-1.5">
                                ${Object.keys(thematic.lda_keywords || {}).map(kw => `
        ...
        `;
    } catch (error) { showToast('Inspection failed', 'error'); }
}
